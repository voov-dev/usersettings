<template>
	<div id="app-content" class="user-list-grid" @scroll.passive="onScroll">
		<Modal v-if="showConfig.showNewUserForm && isUserLimits()" size="small" @close="closeModal">
			<form id="new-user"
				:disabled="loading.all"
				class="modal__content"
				@submit.prevent="createUser">
				<h2>{{ t('usersettings','New user') }}</h2>
				<input id="newusername"
					ref="newusername"
					v-model="newUser.id"
					:disabled="settings.newUserGenerateUserID"
					:placeholder="settings.newUserGenerateUserID
						? t('usersettings', 'Will be autogenerated')
				...
				<input id="newdisplayname"
					...
					:placeholder="t('usersettings', 'Display name')"
					autocapitalize="none"
					autocomplete="off"
					autocorrect="off"
					class="modal__item"
				...
				<div v-if="showConfig.showLanguages" class="languages modal__item">
					<Multiselect v-model="newUser.language"
						:allow-empty="false"
						:options="languages"
						:placeholder="t('usersettings', 'Default language')"
						class="multiselect-vue"
						group-label="label"
				...
				<div class="user-actions">
					<Button id="newsubmit"
						...
						native-type="submit"
						value="">
						{{ t('usersettings', 'Add a new user') }}
					</Button>
				</div>
			</form>
		</Modal>
		<Modal v-else-if="showConfig.showNewUserForm && !isUserLimits()" size="small" @close="closeModal">
			<div class="modal__content">
				<h2>{{ t('usersettings','User limit reached') }}</h2>
				<p>{{ t('usersettings','The limit of users has been reached, to increase the number of users, select an advanced plan.') }}</p>
			</div>
		</Modal>
		<div 
			id="grid-header"
			...
			<div id="headerGroups" class="groups">
				{{ t('usersettings', 'Groups') }}
			</div>
			<div v-if="subAdminsGroups.length>0 && settings.isAdmin"
				id="headerSubAdmins"
				class="subadmins">
				{{ t('usersettings', 'Group admin for') }}
			</div>
			<div id="headerQuota" class="quota">
				{{ t('usersettings', 'Quota') }}
			</div>
			...
			<div v-if="showConfig.showUserBackend || showConfig.showStoragePath"
				...
				<div v-if="showConfig.showUserBackend" class="userBackend">
					{{ t('usersettings', 'User backend') }}
				</div>
				<div v-if="showConfig.showStoragePath"
					class="subtitle storageLocation">
					{{ t('usersettings', 'Storage location') }}
				</div>
			</div>
			<div v-if="showConfig.showLastLogin"
				class="headerLastLogin lastLogin">
				{{ t('usersettings', 'Last login') }}
			</div>
		...
		<InfiniteLoading ref="infiniteLoading" @infinite="infiniteHandler">
			...
			<div slot="no-results">
				<div id="emptycontent">
					...
					<h2>{{ t('usersettings', 'No users in here') }}</h2>
				</div>
			</div>
		</InfiniteLoading>
	</div>
...
const unlimitedQuota = {
	id: 'none',
	label: t('usersettings', 'Unlimited'),
}
const defaultQuota = {
	id: 'default',
	label: t('usersettings', 'Default quota'),
}
const newUser = {
	id: '',
	displayName: '',
	...
	language: {
		code: 'en',
		name: t('usersettings', 'Default language'),
	},
}
...
export default {
	...
	computed: {
		...
		languages() {
			return [
				{
					label: t('usersettings', 'Common languages'),
					languages: this.settings.languages.commonLanguages,
				},
				{
					label: t('usersettings', 'Other languages'),
	...
	mounted() {
		if (!this.settings.canChangePassword) {
			OC.Notification.showTemporary(t('usersettings', 'Password change is disabled because the master key is disabled'))
		}
	...
	methods: {
		...
		isUserLimits() {
			return this.users.length < maxUserCount
		}
	},
}
</script>
