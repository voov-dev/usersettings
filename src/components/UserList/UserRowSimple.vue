			...
			<div v-if="canEdit && !loading.all" class="toggleUserActions">
				<Actions>
					<ActionButton icon="icon-rename" @click="toggleEdit">
						{{ t('usersettings', 'Edit User') }}
					</ActionButton>
				</Actions>
				<div class="userPopoverMenuWrapper">
					<button v-click-outside="hideMenu"
						class="icon-more"
						:aria-label="t('usersettings', 'Toggle user actions menu')"
	...
	computed: {
		...
		usedSpace() {
			if (this.user.quota.used) {
				return t('usersettings', '{size} used', { size: OC.Util.humanFileSize(this.user.quota.used) })
			}
			return t('usersettings', '{size} used', { size: OC.Util.humanFileSize(0) })
		},
		canEdit() {
		...
		userQuota() {
			...
			if (quota === 'none' || quota === -3) {
				return t('usersettings', 'Unlimited')
			} else if (quota >= 0) {
