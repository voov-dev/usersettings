<template>
	...
	<div v-if="Object.keys(user).length ===1" :data-id="user.id" class="row">
		...
		<div class="obfuscated">
			{{ t('usersettings','You do not have permissions to see the details of this user') }}
		</div>
	</div>
	...
	<div v-else
		...
		<form v-if="settings.canChangePassword && user.backendCapabilities.setPassword"
			...
			<input :id="'password'+user.id+rand"
				...
				:placeholder="t('usersettings', 'Add new password')"
				autocapitalize="off"
		...
		<form :class="{'icon-loading-small': loading.mailAddress}"
			...
			<input :id="'mailAddress'+user.id+rand"
				...
				:placeholder="t('usersettings', 'Add new email address')"
				:value="user.email"
				autocapitalize="off"
				autocomplete="new-password"
				autocorrect="off"
		...
		<div :class="{'icon-loading-small': loading.groups}" class="groups">
			<Multiselect :close-on-select="false"
				...
				:placeholder="t('usersettings', 'Add user to group')"
				:tag-width="60"
				...
				@select="addUserGroup"
				@tag="createGroup">
				<span slot="noResult">{{ t('usersettings', 'No results') }}</span>
			</Multiselect>
		</div>
		<div v-if="subAdminsGroups.length>0 && settings.isAdmin"
			...
			<Multiselect :close-on-select="false"
				...
				:placeholder="t('usersettings', 'Set user as admin for')"
				:tag-width="60"
		...
		<div v-tooltip.auto="usedSpace"
			...
			<Multiselect :allow-empty="false"
				...
				:placeholder="t('usersettings', 'Select user quota')"
				:taggable="true"
				:value="userQuota"
				class="multiselect-vue"
				label="label"
		...
		<div v-if="showConfig.showLanguages"
			...
			<Multiselect :allow-empty="false"
				...
				:options="languages"
				:placeholder="t('usersettings', 'No language set')"
				:value="userLanguage"
				class="multiselect-vue"
				group-label="label"
	...
	computed: {
		...
		userActions() {
			const actions = [
				{
					icon: 'icon-delete',
					text: t('usersettings', 'Delete user'),
					action: this.deleteUser,
				},
				{
					icon: 'icon-delete',
					text: t('usersettings', 'Wipe all devices'),
					action: this.wipeUserDevices,
				},
				{
					icon: this.user.enabled ? 'icon-close' : 'icon-add',
					text: this.user.enabled ? t('usersettings', 'Disable user') : t('usersettings', 'Enable user'),
			...
			if (this.user.email !== null && this.user.email !== '') {
				actions.push({
					icon: 'icon-mail',
					text: t('usersettings', 'Resend welcome email'),
	...
	methods: {
		...
		wipeUserDevices() {
			...
			OC.dialogs.confirmDestructive(
				t('usersettings', 'In case of lost device or exiting the organization, this can remotely wipe the Nextcloud data from all devices associated with {userid}. Only works if the devices are connected to the internet.', { userid }),
				t('usersettings', 'Remote wipe of devices'),
				{
					type: OC.dialogs.YES_NO_BUTTONS,
					confirm: t('usersettings', 'Wipe {userid}\'s devices', { userid }),
					confirmClasses: 'error',
					cancel: t('usersettings', 'Cancel'),
				},
				(result) => {
					if (result) {
		...
		deleteUser() {
			const userid = this.user.id
			OC.dialogs.confirmDestructive(
				t('usersettings', 'Fully delete {userid}\'s account including all their personal files, app data, etc.', { userid }),
				t('usersettings', 'Account deletion'),
				{
					type: OC.dialogs.YES_NO_BUTTONS,
					confirm: t('usersettings', 'Delete {userid}\'s account', { userid }),
					confirmClasses: 'error',
					cancel: t('usersettings', 'Cancel'),
				},
				(result) => {
